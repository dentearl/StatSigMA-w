...
bool compute_branch_parameters(Tree* node, Tree* parent, const blocks_type& blocks, 
...
#ifdef DEBUG
    cout << "# Computing branch parameters for node number " << node->leaf_startnum << " " 
         << node->leaf_endnum << endl;
#endif
...
#ifdef DEBUG
    cout << "# Comparison of number of tuples all=" << num_tuples << " " << "block_len= " 
         << blocks.chain[0]->length << endl;
#endif
    if (num_tuples < blocks.chain[0]->length * blocks.chain[0]->length) {
        ...
        */
#ifdef DEBUG
        cout << "# Computing sequence specific scores max(N^2," << g_options.TOTAL_NUM_TUPLES 
        ...
        int* good_tuple1 = (int *)malloc(sizeof(int) * g_options.MAX_BLOCK_SIZE);
        int* good_tuple2 = (int *)malloc(sizeof(int) * g_options.MAX_BLOCK_SIZE);
        int good1 = 0, good2 = 0;
        ...
        if ((good1 > 0) && (good2 > 0)) {      
            for (int tp = 0; tp < g_options.TOTAL_NUM_TUPLES; tp++) {
...
void scan_alignFile(char* mafFile) {
    ...
    CHR_LEN = 0;
    ALIGN_LEN = 0;
    int tempSize = 10 * g_options.MAX_BLOCK_SIZE;
    ...
    while (ifs) {
        ...
        if ((strncmp(buffer, "a score=", 8) == 0) || (!ifs)) {
            // make sure g_options.MAX_BLOCK_SIZE >= actual alignment block size
            if (g_options.MAX_BLOCK_SIZE < cur_len)
                g_options.MAX_BLOCK_SIZE = cur_len;
...
double compute_pvalue_branch(Tree* node, Tree* parent, blocks_type& blocks, char* filename) {
    ...
    cout << "Max block size = " << g_options.MAX_BLOCK_SIZE << endl;
#endif
    ...
    srand(time(NULL));
    double *K_arr = new double[g_options.TOTAL_ITERATE_PARAM];
    double *lambda_arr = new double[g_options.TOTAL_ITERATE_PARAM];
    double *H_arr = new double[g_options.TOTAL_ITERATE_PARAM];
    blocks_type* blocks_param = new blocks_type;
    ifstream ifs1(filename);
    // int runs = 0;
  
    float rate = ALIGN_LEN / (float)g_options.TOTAL_ITERATE_PARAM;
    ...
    while ((blocks_param->read_single_from_maf(ifs1)) 
           && (sim_count < g_options.TOTAL_ITERATE_PARAM)) {
    ...
    pair <int, int> *limit = new pair <int, int>[g_options.MAX_SEGMENTS];
    pair <double, double> *score_left_right = new pair <double, double>[g_options.MAX_SEGMENTS];
    int count = 0;
            ...
            */
            ...
            if (score > 0) {
                ...
                if ((max_j==-1)||(score_left_right[all_good_segments + max_j].second >= cur_score)) {
                    ...
                    if (block_good_segments + all_good_segments < g_options.MAX_SEGMENTS - 1) {
                        block_good_segments++;
                        /*
                          #ifdef DEBUG
